...
// files and creates and stores language objects, and provides API to access
// them.
//
// Use languagemanager.GetDefault() to retrieve the default instance of
// GtkSourceLanguageManager, and languagemanager.GuessLanguage to get a language
...
// manager instance then use languagemanager.GetDefault() instead.
//
...
func (lm *LanguageManager) AppendSearchPath(path string) {
	...
	C.gtk_source_language_manager_append_search_path(_arg0, _arg1)
	runtime.KeepAlive(lm)
	runtime.KeepAlive(path)
}

// Language gets the language identified by the given id in the language
// manager.
//
// The function takes the following parameters:
//
...
// The function returns the following values:
//
//   - utf8s: NULL-terminated array containing a list of language files
...
// GuessLanguage picks a language for given file name and content type,
// according to the information in lang files.
//
// Either filename or content_type may be NULL. This function can be used as
// follows:
//
...
func (lm *LanguageManager) PrependSearchPath(path string) {
	...
	C.gtk_source_language_manager_prepend_search_path(_arg0, _arg1)
	runtime.KeepAlive(lm)
	runtime.KeepAlive(path)
}
...
// are loaded for the first time. In practice to set a custom search path for a
// GtkSourceLanguageManager, you have to call this function right after creating
// it.
//
// Since GtkSourceView 5.4 this function will allow you to provide paths in the
// form of "resource:///" URIs to embedded GResources. They must contain the
