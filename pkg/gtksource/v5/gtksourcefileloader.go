...
//
// After a file loading, the buffer is reset to the contents provided by
// the gio.File or gio.InputStream, so the buffer is set as “unmodified”,
// that is, gtk.TextBuffer.SetModified() is called with FALSE. If the contents
// isn't saved somewhere (for example if you load from stdin), then you
// should probably call gtk.TextBuffer.SetModified() with TRUE after calling
...
// NewFileLoader creates a new GtkSourceFileLoader object. The contents is read
...
//
// If the contents has been loaded, the following file properties will be
...
//
// 1. If set, the file's encoding as returned by file.GetEncoding. 2. The
// default candidates as returned by encoding.GetDefaultCandidates().
//
// The function takes the following parameters:
//
